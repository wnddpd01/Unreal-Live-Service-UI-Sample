#include "RefreshUIEventContractsCommandlet.h"
...
namespace
{
    ...
    FString MakeCppIdentifier(const FString& RawValue)
    {
        FString Identifier;
        Identifier.Reserve(RawValue.Len());
        ...
        {
            Identifier.InsertAt(0, TEXT("_"));
        }

        return Identifier;
    }

    bool GenerateCppBindingHeader(
        const TMap<FString, TSharedPtr<FJsonObject>>& EventsById,
        const FString& OutputPath,
        FString& OutError
    )
    {
        TArray<FString> EventIds;
        EventsById.GenerateKeyArray(EventIds);
        EventIds.Sort();

        TSet<FString> GeneratedSymbols;
        FString HeaderText;
        HeaderText += TEXT("#pragma once\n\n");
        HeaderText += TEXT("#include \"CoreMinimal.h\"\n");
        HeaderText += TEXT("#include \"UIEventDeclarationMacros.h\"\n\n");
        HeaderText += TEXT("// Generated by RefreshUIEventContracts. Do not edit by hand.\n\n");

        for (const FString& EventId : EventIds)
        {
            TArray<FString> Parts;
            EventId.ParseIntoArray(Parts, TEXT("."), true);
            if (Parts.Num() < 2)
            {
                OutError = FString::Printf(
                    TEXT("Cannot generate C++ binding for invalid EventId '%s'."),
                    *EventId
                );
                return false;
            }

            HeaderText += TEXT("namespace UIEvents\n{\n");

            FString SymbolPath = TEXT("UIEvents");
            for (int32 PartIndex = 0; PartIndex < Parts.Num() - 1; ++PartIndex)
            {
                const FString NamespaceName = MakeCppIdentifier(Parts[PartIndex]);
                SymbolPath += TEXT("::") + NamespaceName;
                HeaderText += FString::Printf(TEXT("namespace %s\n{\n"), *NamespaceName);
            }

            const FString ConstantName = MakeCppIdentifier(Parts.Last());
            SymbolPath += TEXT("::") + ConstantName;
            if (GeneratedSymbols.Contains(SymbolPath))
            {
                OutError = FString::Printf(
                    TEXT("Generated C++ symbol collision '%s' for EventId '%s'."),
                    *SymbolPath,
                    *EventId
                );
                return false;
            ...
            GeneratedSymbols.Add(SymbolPath);
            HeaderText += FString::Printf(
                TEXT("inline const FName %s = TEXT(\"%s\");\n"),
                *ConstantName,
                *EventId
            );
        ...
        if (!FFileHelper::SaveStringToFile(HeaderText, *OutputPath))
        {
            OutError = FString::Printf(TEXT("Failed to write %s."), *OutputPath);
            return false;
        }
...
URefreshUIEventContractsCommandlet::URefreshUIEventContractsCommandlet()
{
    IsClient = false;
    IsEditor = true;
    IsServer = false;
...
int32 URefreshUIEventContractsCommandlet::Main(const FString& Params)
{
    TMap<FString, TSharedPtr<FJsonObject>> EventsById;
    ...
    const FString ProjectName = GetHostProjectName();
    if (ProjectName.IsEmpty())
    {
        UE_LOG(LogTemp, Error, TEXT("Failed to resolve host project name."));
        return 1;
    }
    ...
    const FString ManifestPath = FPaths::Combine(OutputDir, TEXT("UIEvents.json"));
    const FString CppBindingPath = FPaths::Combine(OutputDir, TEXT("GeneratedUIEventIds.h"));

    if (!FFileHelper::SaveStringToFile(OutputJson, *ManifestPath))
    {
        UE_LOG(LogTemp, Error, TEXT("Failed to write %s."), *ManifestPath);
        return 1;
    }

    FString BindingError;
    if (!GenerateCppBindingHeader(EventsById, CppBindingPath, BindingError))
    {
        UE_LOG(LogTemp, Error, TEXT("%s"), *BindingError);
        return 1;
    }
    ...
    UE_LOG(
        LogTemp,
        Display,
        TEXT("Refreshed %d UI event contracts into %s and %s."),
        EventsById.Num(),
        *ManifestPath,
        *CppBindingPath
    );
